...
class GHLService {
  ...
  async makeRequest(method, url, dataOrOptions = {}, retryCount = 0) {
    try {
      ...
      if (error.response?.status === 429 && retryCount < this.maxRetries) {
        ...
        console.log(
          `Rate limited (429). Esperando ${waitTime / 1000}s antes de reintentar... (intento ${retryCount + 1}/${this.maxRetries})`,
        );
        await this.sleep(waitTime);
        return this.makeRequest(method, url, dataOrOptions, retryCount + 1);
      }
      throw error;
    }
  }
  ...
  async getOpportunities(startDate = null, endDate = null) {
    ...
    if (this._cachedOpportunities && this._cacheTimestamp) {
      ...
      if (age < this._cacheTTL) {
        ...
        console.log(
          `⚡ Cache memoria: ${this._cachedOpportunities.length} oportunidades (edad: ${ageSec}s)`,
        );
  ...
  async _fetchAllOpportunities() {
    try {
      ...
      while (hasMore) {
        ...
        console.log(
          `  Página ${pageCount}: ${opportunities.length} (Total: ${allOpportunities.length})`,
        );
      ...
      console.log(
        `✅ Total oportunidades: ${allOpportunities.length} (cacheado por 5 min)`,
      );
      ...
      if (error.response?.status === 429) {
        throw new Error(
          "La API de Go High Level está limitando las solicitudes. Por favor espera unos minutos e intenta de nuevo.",
        );
      }
      throw error;
    }
  ...
  async getPipelineStages() {
    try {
      const response = await this.makeRequest(
        "get",
        `/opportunities/pipelines/${this.pipelineId}`,
        {
