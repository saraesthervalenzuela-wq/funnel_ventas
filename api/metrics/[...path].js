// ============================================================
// api/metrics/[...path].js — Vercel Serverless Function
// Port 1:1 de netlify/functions/metrics.js.
// El query layer de supabase fue reemplazado por lib/db.js (Neon).
// El cálculo de métricas y el routing interno se mantienen idénticos.
// ============================================================
        ...
        if (sorted[i].direction === "inbound") {
          for (let j = i + 1; j < sorted.length; j++) {
            if (sorted[j].direction === "outbound") {
              const diffMinutes =
                (new Date(sorted[j].dateAdded) -
                  new Date(sorted[i].dateAdded)) /
                (1000 * 60);
...
function buildFunnelMetrics(opportunities) {
  ...
  const leadsCalificados = opportunities.filter(
    (o) =>
      o.pipelineStageId !== stageIds.nuevoLead &&
      o.pipelineStageId !== stageIds.noInteresado,
  ).length;

  const stagesAgendadas = [
    stageIds.valoracionVirtual,
    stageIds.vvReagendada,
    stageIds.vvNoContesto,
    stageIds.valoracionRealizada,
    stageIds.seguimientoCierre,
    stageIds.depositoRealizado,
    stageIds.fechaCirugia,
  ];
  const agendadasValoracion = opportunities.filter((o) =>
    stagesAgendadas.includes(o.pipelineStageId),
  ).length;
  ...
  const valoradasCotizacion = opportunities.filter((o) =>
    stagesValoradas.includes(o.pipelineStageId),
  ).length;
  ...
  const oportunidadesCierre = opportunities.filter((o) =>
    stagesCierre.includes(o.pipelineStageId),
  );
  const cierreAlta = oportunidadesCierre.filter(
    (o) => (parseFloat(o.monetaryValue) || 0) > 50000,
  ).length;
  const cierreMedia = oportunidadesCierre.filter((o) => {
    const v = parseFloat(o.monetaryValue) || 0;
    return v > 0 && v <= 50000;
  }).length;

  const stagesDeposito = [stageIds.depositoRealizado, stageIds.fechaCirugia];
  const depositosRealizados = opportunities.filter((o) =>
    stagesDeposito.includes(o.pipelineStageId),
  );
  const totalDepositos = depositosRealizados.reduce(
  ...
  const depositosCampanas = depositosRealizados.filter((o) => {
    ...
    return (
      ...
      tags.includes("fb-ad") ||
      tags.includes("instagram-ad")
    );
  });
  const totalDepositosCampanas = depositosCampanas.reduce(
    (s, o) => s + (parseFloat(o.monetaryValue) || 0),
    0,
  );
...
function buildStageDistribution(opportunities) {
  ...
  opportunities.forEach((opp) => {
    if (distribution[opp.pipelineStageId]) {
      ...
      distribution[opp.pipelineStageId].value +=
        parseFloat(opp.monetaryValue) || 0;
    }
  });
  return stageOrder.map((id) => ({
    stageId: id,
    stage: stageNames[id] || id,
    count: distribution[id].count,
    value: distribution[id].value,
  }));
}
...
function buildAverageTimes(opportunities) {
  ...
  closed.forEach((o) => {
    const days = Math.ceil(
      (new Date(o.updatedAt || o.lastStatusChangeAt) - new Date(o.createdAt)) /
        (1000 * 60 * 60 * 24),
    );
  ...
  const avg =
    tiempos.length > 0
      ...
      : 0;
  return {
    promedioTiempoCierre: avg,
    oportunidadesAnalizadas: closed.length,
...
function getChannel(source, tagsJoined) {
  ...
  if (tagsJoined.includes("fb-ad-lead") || tagsJoined.includes("fb-ad"))
    return "Facebook Ads";
  if (
    tagsJoined.includes("instagram-ad-lead") ||
    tagsJoined.includes("instagram-ad")
  )
  ...
  if (
    s.includes("whatsapp") ||
    tagsJoined.includes("inbound whatsapp") ||
    tagsJoined.includes("wa:") ||
    tagsJoined.includes("wazz") ||
    tagsJoined.includes("whatsapp")
  )
    return "WhatsApp";
  if (
    s.includes("email") ||
    s.includes("correo") ||
    tagsJoined.includes("correo")
  )
    return "Correo";
...
function buildSourceMetrics(opportunities) {
  ...
  opportunities.forEach((opp) => {
    ...
    if (stageId !== stageIds.nuevoLead && stageId !== stageIds.noInteresado)
    ...
    if (isDeposito) {
      ...
      channelMetrics[channel]._campaigns[campaignName].valorTotal +=
        parseFloat(opp.monetaryValue) || 0;
    }
  });
  return Object.entries(channelMetrics)
    .map(([source, m]) => ({
      source,
      total: m.total,
      calificados: m.calificados,
      valoraciones: m.valoraciones,
      depositos: m.depositos,
      valorTotal: m.valorTotal,
      tasaConversion:
        m.total > 0 ? ((m.depositos / m.total) * 100).toFixed(2) : 0,
      campaigns: Object.entries(m._campaigns)
        .map(([campaign, cm]) => ({
          campaign,
          ...cm,
          tasaConversion:
            cm.total > 0 ? ((cm.depositos / cm.total) * 100).toFixed(2) : 0,
        }))
...
function buildDailyTrend(opportunities) {
  ...
  opportunities.forEach((opp) => {
    const d = new Date(
      opp.lastStageChangeAt || opp.lastStatusChangeAt || opp.createdAt,
    );
    const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")}`;
    if (!dailyData[key])
      dailyData[key] = { leads: 0, valoraciones: 0, depositos: 0 };
    dailyData[key].leads++;
...
function buildOwnerMetrics(opportunities) {
  ...
  return Object.entries(OWNERS).map(([ownerId, name]) => {
    ...
    const calificados = ownerOpps.filter(
      (o) =>
        o.pipelineStageId !== stageIds.nuevoLead &&
        o.pipelineStageId !== stageIds.noInteresado,
    ).length;
    const cierres = ownerOpps.filter((o) =>
      stagesDeposito.includes(o.pipelineStageId),
    ).length;
    const tasaConversion =
      totalLeads > 0 ? ((cierres / totalLeads) * 100).toFixed(2) : 0;
...
function buildCurrentSnapshot(allOpportunities) {
  ...
  const build = (id) => ({
    count: countByStage[id] || 0,
    stale: staleByStage[id] || 0,
  });
...
// ============================================
// HANDLER PRINCIPAL (firma Vercel)
// ============================================
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // /current-stages — snapshot actual sin filtro de fecha (lee de DB)
    if (path === "current-stages") {
      const allRows = await db.getOpportunitiesLight();

      const opps = allRows.map((row) => ({
        pipelineStageId: row.pipeline_stage_id,
        lastStageChangeAt:
          row.raw_json?.lastStageChangeAt || row.raw_json?.lastStatusChangeAt,
    ...
    // /summary — endpoint principal (usa snapshots para evitar timeout)
    if (path === "summary" || path === "" || !path) {
      ...
      return res
        .status(200)
        .json({ success: true, dateRange, source: "database", data });
    }

    // /sync-info
    if (path === "sync-info") {
      const data = await db.getLastSyncInfo();
      return res.status(200).json({ success: true, data });
    }
